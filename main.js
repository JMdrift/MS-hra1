/* ═══════════ herní logika ═══════════ */
const has = n => Object.entries(n).every(([k,v]) => getRes(k) >= v);
const pay = n => Object.entries(n).forEach(([k,v]) => addRes(k, -v));
...
function syncPlatforms() {
  ...
  PLATFORMS.forEach(pl => {
    ...
    /* horské platformy odemyká konkrétní stavba, ne jen level */
    if (pl.req && !gateDone(pl.req)) return;
...
function collect(id, quiet) {
  ...
  floats.push({ x:c.x, y:c.y, t:`+${take} · +${g} XP`, life:1.1, c:C.green, w:S.world });
...
/* stejné, ale jen městské budovy — automatika na hory nesahá, tam sklízíš sám */
function collectCityRent() {
  let t = 0;
  PIDS.forEach(id => { if (PARC[id].plat >= MTN_FIRST) return;
...
function finishPhase(id) {
  ...
  floats.push({ x:c.x, y:c.y, t:`+${p.xp} XP`, life:1.3, c:C.amber, w:S.world });
  if (st.phase >= D.ph.length) {
    st.done = true; bump('built');
    if (PARC[id].plat >= MTN_FIRST) setTimeout(syncPlatforms, 0);
    ...
    if (D && D.sea) {                         // první stavba citlivá na sezónu
      showEvent({ once:'sezona', kick:'Sezóna', tone:'sk', title:'V horách se střídá léto a zima',
        desc:'Od téhle chvíle záleží na ročním období — nahoře běží pruh s odpočtem.',
        lines:[['Zima','sjezdovka jede naplno, chaty jsou plné'],
               ['Léto','turisté chodí na rozhlednu a do lázní'],
               ['Střídá se', `po ${(SEASON_LEN/60000)|0} minutách`]] });
    }
    if (D && D.world) {                       // nádraží otevře horský kraj
      syncPlatforms();
      showEvent({ kick:'Hotovo', tone:'ok', title:'Nádraží stojí',
        desc:'Vlaky jsou připravené. Za hřebenem na tebe čeká <b>horský kraj</b>.',
        lines:[['Tlačítko SVĚT','vlevo dole tě přepne mezi městem a horami'],
               ['Peníze','platí v obou světech'],
               ['Suroviny','horské se musí vytěžit na místě']] });
    }
    if (isPump(id)) {
...
function finishNodePhase(id) {
  ...
  if (n.ph >= u.ph.length) {
    ...
    if ((nodeDef(id).plat||0) >= MTN_FIRST) syncPlatforms();
...
function grabEvent(i) {
  ...
  else { const room = capOf(e.res) - getRes(e.res), q = Math.min(e.q, room);
...
/* poruchy — kde stavba stojí, aby tam mohli dojet hasiči */
function faultPos(key) {
  const id = key.slice(2);
  if (key[0] === 'n') { const d = nodeDef(id); return d ? [d.gx, d.gy] : null; }
  const p = PARC[id]; return p ? [p.gx, p.gy] : null;
}
/* zaplatíš — vyjedou hasiči, po cestě se to opraví */
function repair(key) {
  const f = S.faults[key]; if (!f || f.fix) return;
  const c = repairCost(key);
  if (S.money < c) { toast('Nemáš dost peněz na opravu'); return; }
  S.money -= c; f.fix = Date.now() + REPAIR_DRIVE; f.dur = REPAIR_DRIVE;
  const pos = faultPos(key); if (pos) spawnFire(pos[0], pos[1], REPAIR_DRIVE);
  toast('Opravář vyjel · ' + fmt(c));
  renderTop(); save();
}
/* dojeli — porucha zmizí */
function finishRepairs() {
  const now = Date.now();
  Object.keys(S.faults).forEach(k => { const f = S.faults[k];
    if (f.fix && now >= f.fix) { delete S.faults[k]; bump('repairs');
      toast('Opraveno: ' + f.n); renderRes(); save(); }
  });
}
...
/* zakázka právě doběhla — jen upozorní, nevyzvedne (to udělá tap nebo automatika) */
function orderDone(a, i) {
  toast(a.client + ' — zakázka je hotová, vyzvedni si ji');
  const zb = document.querySelector('[data-s="zakazky"]');
  if (zb) zb.classList.add('dot');
}
/* vyzvedne úplně všechny dokončené zakázky najednou */
function claimAllDone() {
  let sum = 0, xp = 0, n = 0;
  for (let i = S.active.length - 1; i >= 0; i--) {
    const a = S.active[i];
    if (Date.now() < a.end) continue;
    sum += a.pay; xp += a.xp; n++;
    S.active.splice(i, 1);
  }
  if (n < 1) return 0;
  S.money += sum; addXP(xp); S.seen.order1 = 1;
  toast(`Vyzvednuto ${n} zakázek · +${fmt(sum)}`);
  renderTop(); save();
  return sum;
}
...
function enqueue(id, out, amt, quiet) {
  ...
  Object.entries(rec).forEach(([k,v]) => addRes(k, -q*v));
...
function tap(p) {
  ...
  /* skutečné parcely mají přednost před popiskem zamčené sousední platformy */
...
/* ═══════════ AUTOMATIZACE — dispečink, správa, údržba ═══════════
   Pracují jen ve městě. V horách se sklízí ručně, tam automatika nesahá. */
function autoReconnect() {
  ['a1','a2','a3'].forEach(id => { const st = S.plot[id], D = defOf(id);
    if (!D || !st || !st.done || !st.off) return;
    const dr = (D.draw && D.draw[Math.min(st.hl,D.draw.length)-1]) || 0;
    const wr = (D.wdraw && D.wdraw[Math.min(st.hl,D.wdraw.length)-1]) || 0;
    if (powerFree() >= dr && waterFree() >= wr) { st.off = 0;
      toast(D.name + ' je zpátky v síti'); renderTop(); save(); }
  });
}
function autoMineTick(now) {
  const ms = autoMineMs();
  if (!ms || now - (S.autoT.mine||0) < ms) return;
  S.autoT.mine = now;
  let got = 0;
  Object.keys(NODE_DEF).forEach(id => {
    if ((nodeDef(id).plat||0) >= MTN_FIRST) return;   // hory sklízí hráč sám
    if (S.nodes[id].buf > 0) got += collect(id, true) || 0;
  });
  if (got) toast('Dispečink svezl ' + got + ' surovin');
}
function autoRentTick(now) {
  const ms = autoRentMs();
  if (!ms) return;                                        // bez správy se nic nevyzvedává samo
  if (S.active.some(a => now >= a.end)) claimAllDone();    // správa umí i hotové zakázky
  if (now - (S.autoT.rent||0) < ms) return;
  S.autoT.rent = now;
  const r = collectCityRent();                             // jen městský nájem
  if (r > 0) toast('Správa vybrala nájem ' + fmt(r));
}
function autoFixTick(now) {
  const mul = autoFixMul();
  if (!mul) return;
  Object.keys(S.faults).forEach(k => {
    if (k[0] === 'p' && PARC[k.slice(2)] && PARC[k.slice(2)].plat >= MTN_FIRST) return;
    if (k[0] === 'n' && (nodeDef(k.slice(2)).plat||0) >= MTN_FIRST) return;
    const f = S.faults[k];
    if (f.fix) return;
    const cost = Math.round(repairCost(k) * mul);
    if (S.money < cost) return;
    S.money -= cost; f.fix = now + REPAIR_DRIVE; f.dur = REPAIR_DRIVE;
    const pos = faultPos(k); if (pos) spawnFire(pos[0], pos[1], REPAIR_DRIVE);
    toast('Údržba vyjela · ' + fmt(cost)); renderTop();
  });
}
/* volá se z hlavní smyčky; obalené, ať jedna chyba nezastaví zbytek hry */
/* diagnostický tlak na tlačítko v panelu — spustí automatiku hned,
   bez ohledu na časovač, a řekne přesně, co se stalo nebo proč ne */
function forceAutoRun(id) {
  const D = defOf(id);
  if (!D) return;
  autoReconnect();
  const st = S.plot[id];
  if (st.off) { toast(`Pořád odpojeno — volný proud ${powerFree()} MW, voda ${waterFree()} m³`); return; }
  if (isBroken('p:'+id)) { toast(`Porouchané: ${S.faults['p:'+id].n} — oprava stojí ${fmt(repairCost('p:'+id))}`); return; }
  if (D.autoMine) {
    let got = 0;
    Object.keys(NODE_DEF).forEach(nid => {
      if ((nodeDef(nid).plat||0) >= MTN_FIRST) return;
      if (S.nodes[nid].buf > 0) got += collect(nid, true) || 0;
    });
    toast(got ? `Fungovalo — sveženo ${got} surovin` : 'Fungovalo, ale nic nebylo ve vyrovnávací paměti k svezení');
    return;
  }
  if (D.autoRent) {
    if (S.active.some(a => Date.now() >= a.end)) claimAllDone();
    const r = collectCityRent();
    toast(r>0 ? `Fungovalo — vybráno ${fmt(r)}` : 'Fungovalo, ale žádná budova neměla naakumulovaný nájem');
    return;
  }
  if (D.autoFix) {
    const before = faultCount();
    autoFixTick(Date.now());
    toast(before===0 ? 'Fungovalo — momentálně ale nic není rozbité' : 'Fungovalo — údržba vyjela, zkontroluj Stavby');
  }
}
function autoTick(now) {
  try { autoReconnect(); } catch (e) { console.error('autoReconnect', e); }
  try { autoMineTick(now); } catch (e) { console.error('autoMineTick', e); }
  try { autoRentTick(now); } catch (e) { console.error('autoRentTick', e); }
  try { autoFixTick(now); } catch (e) { console.error('autoFixTick', e); }
}
...
setInterval(() => { try {
  ...
  Object.keys(NODE_DEF).forEach(id => {
    ...
    if (nodePowered(id) && nodeOk(id)) nodeMakes(id).forEach(out => {
      if ((n.make[out]||0) > 0 && now >= (n.mEnd[out]||0)) {
        if (getRes(out) < capOf(out)) { addRes(out,1); n.make[out]--; bumpRes('made', out, 1); renderRes();
  ...
  if (S.world === 1 && !mtnOpen()) { S.world = 0; WB = worldBounds(); clampPan();
    toast('Horský kraj ještě není otevřený'); }
  finishRepairs();
  if (mtnOpen()) {
  ...
  bump('play', 160);
  saveSoon();
} catch (e) { console.error('hlavní smyčka', e); } }, 160);
...
const QUESTS = [
  ...
  { id:'svet', done:()=>!hasWorld() || !!S.seen.svet || mtnOpen(),
    t:'Mrkni za hřeben', d:'Tlačítko SVĚT vlevo dole tě vezme do horského kraje',
    p:()=>'', go:()=>{ S.seen.svet = 1; save(); worldSheet(); } },
  /* ─── horský kraj ─── */
  { id:'mtn0', done:()=>true, t:'Objev horský kraj',
    d:'Tlačítko SVĚT vlevo dole tě vezme za hřeben',
    p:()=>'', go:()=>{ syncPlatforms(); worldSheet(); } },
  { id:'mtnStore', done:()=>!mtnOpen() || mtnStoreCap() > 0,
    t:'Postav horský sklad', d:'Je zdarma — bez něj nemáš kam vozit ani co těžit',
    p:()=>bPh('mt2'), target:()=>Tg('mt2'), go:()=>{ gotoWorld(1); openParcel('mt2'); } },
  { id:'mtLanovka', done:()=>!mtnOpen() || platOpen(21),
    t:'Postav lanovku k lomu', d:'Otevře žulový lom — bez ní se tam nedostaneš',
    p:()=>bPh('mt4'), target:()=>Tg('mt4'), go:()=>{ gotoWorld(1); openParcel('mt4'); } },
  { id:'mtLom', done:()=>!mtnOpen() || S.nodes.zula.lvl > 0,
    t:'Otevři žulový lom', d:'První horská surovina — z města sem nic nedovezeš',
    p:()=>`${S.nodes.zula.ph} / 3`, go:()=>{ cls(); upgradeSheet('zula'); } },
  Object.assign(qNode('kdilna1',1,'Otevři kamenickou dílnu','Z žuly udělá kvádry — bez nich dál nepostavíš nic'),
    { done:()=>!mtnOpen() || S.nodes.kdilna1.lvl>=1 }),
  { id:'mtChata', done:()=>!mtnOpen() || PIDS.some(i=>defOf(i)&&defOf(i).name==='Horská chata'&&S.plot[i].done),
    t:'Postav první chatu', d:'Konečně vlastní nájem v horách — a v zimě vydělá skoro dvakrát tolik',
    p:()=>bPh('mt3'), target:()=>Tg('mt3'), go:()=>{ gotoWorld(1); openParcel('mt3'); } },
  { id:'konec', done:()=>false,
    t: () => mtnOpen() ? 'Rozvíjej hory i město' : 'Vylepšuj první město',
    d: () => mtnOpen()
      ? 'Obě části běží zároveň — přepínej je tlačítkem vlevo dole'
      : 'Horský kraj se dodělává. Zatím máš město, kde je pořád co zlepšovat.',
...
syncLevel();
syncPlatforms();
if (S.world === 1 && !mtnOpen()) S.world = 0;   // ať se nedá uvíznout v prázdnu
WB = worldBounds();
/* kamera na střed právě zobrazeného světa */
(function centerWorld(){
  const sc = Z(), cx = (WB.x0+WB.x1)/2, cy = (WB.y0+WB.y1)/2;
  panX = -(cx-cy)*(TW/2)*sc;
  panY = -(cx+cy)*(TH/2)*sc + (H*.44 - cy0);
  clampPan();
})();
