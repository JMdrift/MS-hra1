...
const cap = () => (SKLAD_UP[Math.min(S.skladLvl, SKLAD_UP.length-1)]||SKLAD_UP[0]).cap;
...
/* městské suroviny jsou společné pro oba světy, horské mají vlastní halu */
const capOf = k => isMtnRes(k) ? mtnStoreCap() : isIron(k) ? ironCap() : cap();
const getRes = k => S.res[k] || 0;
const setRes = (k, v) => { S.res[k] = Math.max(0, v); };
const addRes = (k, n) => setRes(k, getRes(k) + n);
...
/* brána platformy může být parcela (dům) nebo stanice (těžba/dílna) */
const isGateNode = id => !!NODE_DEF[id];
const gateDone = id => isGateNode(id) ? (S.nodes[id] && S.nodes[id].lvl > 0)
  : (S.plot[id] && S.plot[id].done);
const gateName = id => isGateNode(id) ? nodeDef(id).base : PARC[id].name;
const gatePlat = id => isGateNode(id) ? nodeDef(id).plat : PARC[id].plat;
...
const specialDef = id => isPlant(id) ? PLANT : isPump(id) ? PUMP
  : isIronStore(id) ? IRON_STORE : PARC[id].port ? PORT[PARC[id].port].build : null;
...
function orderPool() {
  ...
  const hasKind = k => Object.keys(NODE_DEF).some(i=>S.nodes[i].lvl>=1 && nodeDef(i).kind===k);
  if (hasKind('coal')) out.push('uhli');
  /* horské suroviny se do zakázek zapojí, jakmile je hráč začne těžit */
  if (hasKind('granite')) out.push('zula');
  if (hasKind('slateq'))  out.push('bridlice');
  if (hasKind('quartz'))  out.push('kremen');
  /* zpracované (kvádry/tabule/sklo) přibudou samy — dílny „makes" hlásí přes nodeMakes */
...
function makeOrder(seed) {
  ...
  for (let i = 0; i < cnt; i++) {
    ...
    const divisor = Math.max(1, RL(k).price / 20);       // dražší surovina, menší množství
    let q = Math.max(4, Math.round(base / divisor));
    const lim = Math.max(6, Math.floor(capOf(k)*0.85));   // podle vlastního skladu té suroviny
...
/* horský obchod — stejný mechanismus jako ve městě, jen dražší suroviny */
const mtnPrice = k => Math.round(RL(k).price * BUY_MUL);
/* surovinu jde koupit, teprve když ji hráč umí sám vytěžit/vyrobit */
const MTN_UNLOCK = {
  zula: () => S.nodes.zula.lvl > 0,
  bridlice: () => S.nodes.bridl.lvl > 0,
  kremen: () => S.nodes.krem.lvl > 0,
  kvadr: () => S.nodes.kdilna1.lvl > 0 || S.nodes.kdilna2.lvl > 0,
  tabule: () => S.nodes.krytin1.lvl > 0,
  sklo: () => S.nodes.sklar1.lvl > 0
};
const mtnBuyable = k => !MTN_UNLOCK[k] || MTN_UNLOCK[k]();
...
/* ─── sezóny ─── */
/* na sezóně záleží, až stojí něco, co ji vnímá */
const seasonMatters = () => PIDS.some(id => { const D = defOf(id);
  return D && D.sea && S.plot[id].done; });
...
function load() {
  ...
  /* poškozený časovač (NaN, minus, daleko v budoucnosti) by navždy blokoval automatiku —
     radši ho vrátit na nulu, ať se svoz spustí hned při dalším tiku */
  const nowLoad = Date.now();
  ['mine','rent'].forEach(k => { const v = S.autoT[k];
    if (typeof v !== 'number' || !isFinite(v) || v > nowLoad) S.autoT[k] = 0; });
