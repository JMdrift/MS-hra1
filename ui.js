...
/* ─── HUD ─── */
function resPill(r) {
  const v = S.res[r.k], c = capOf(r.k), full = c > 0 && v >= c;
...
function renderRes() {
  ...
  const showHigh = ALL.some(r=>r.tier===2 && S.res[r.k]>0)
    || Object.keys(NODE_DEF).some(id=>S.nodes[id].lvl>=4);
  const showIron = ironBuilt() || S.nodes.ruda.lvl>0 || S.nodes.hut.lvl>0
    || IRON.some(r=>S.res[r.k]>0);
  const vis = { 0:true, 1:showMat, 2:showHigh, 3:showIron };
  const list = ALL.filter(r => vis[r.tier]);
  $('resrow').innerHTML =
  ...
  const wb = $('wbox');
  if (waterMax() > 0) { wb.style.display='flex';
    const wf = waterFree();
...
function renderWorld() {
  ...
  const show = hasWorld() && mtnOpen() && MODE.v === 'map'
...
function gotoWorld(w) {
  ...
  panX = -(cx-cy)*(TW/2)*sc;
  panY = -(cx+cy)*(TH/2)*sc + (H*.42 - cy0);
  clampPan(); cls(); closeParcel();
...
function renderSeason() {
  const el = $('seabar');
  if (!el) return;
  if (!mtnOpen() || !inMtn()) { el.classList.remove('on'); return; }
  const S2 = season();
  el.classList.add('on');
...
function parcelHTML(id) {
  ...
  if (PARC[id].mtrain) return trainHTML(id);
...
function doneHTML(id,D,st) {
  ...
  if (D.mtnStore) {
    h += `<div class="ss">Horské suroviny se do městských skladů nevejdou —
      ...
      ${MTN.map(r=>`<div class="rw"><div class="ric">${icon(r.ic,r.c,20)}</div>
        <div class="rt"><b>${r.l}</b><i>${S.res[r.k]} / ${mtnStoreCap()} ks</i>
        ${capBar(S.res[r.k],Math.max(1,mtnStoreCap()))}</div></div>`).join('')}`;
  ...
  if (D.autoMine || D.autoRent || D.autoFix) {
    ...
    const txt = D.autoMine ? `Sveze suroviny ze všech stanic <b>každých ${(D.autoMine[i]/1000)|0} s</b>.`
      : D.autoRent ? `Vybere nájem ze všech budov <b>každých ${(D.autoRent[i]/1000)|0} s</b>
        a sama vyzvedne <b>hotové zakázky</b>.`
      : `Opraví každou poruchu sama, ale za <b>${D.autoFix[i].toFixed(1)}× cenu</b> běžné opravy.`;
    h += `<div class="ss">${txt} Nemusíš na nic klikat.</div>`;
    const up = nextHouseUp(id);
...
function ironHTML(id) {
  ...
  let h = `<div class="st">Rudný sklad<span class="tag sky">LVL ${S.iron.lvl}</span></div>
    ...
    ${IRON.map(r=>`<div class="rw"><div class="ric">${icon(r.ic,r.c,20)}</div>
      <div class="rt"><b>${r.l}</b><i>${S.res[r.k]} / ${ironCap()} ks</i>
      ${capBar(S.res[r.k],ironCap())}</div></div>`).join('')}`;
...
function trainHTML(id) {
  const st = S.plot[id];
  if (!st.done) {
    if (st.bEnd) { const cur = TRAIN.ph[st.phase];
      return `<div class="st">Nádražní sklad<span class="tag sky">staví se</span></div>
        ${stripHTML(TRAIN.ph,st.phase)}<div class="now-line"><b>${cur.n}</b></div>
        ${progBar(st.bEnd, st.bDur||TRAIN.dur)}`; }
    const cur = TRAIN.ph[st.phase];
    return `<div class="st">Nádražní sklad<span class="tag">fáze ${st.phase+1} / ${TRAIN.ph.length}</span></div>
      ${st.phase===0?`<div class="ss">Sem přijíždí vlak z města. Naložíš, co chceš převézt,
        a za tři minuty to tu složíš.</div>`:''}
      ${stripHTML(TRAIN.ph,st.phase)}
      <div class="now-line"><b>${cur.n}</b><span>${(TRAIN.dur/1000)|0} s · +${fmtN(cur.xp)} XP</span></div>
      <div class="mats">${needTxt(cur.need)}</div>
      <button class="btn" id="bd">Postavit fázi</button>`;
  }
  const q = S.train.q, capT = trainCap(), load = st.mload || 0;
  const left = S.train.end ? Math.max(0, S.train.end - Date.now()) : 0;
  let h = `<div class="st">Nádražní sklad<span class="tag sky">${capT} beden na soupravu</span></div>
    <div class="ss">Vlak z města vozí <b>staré suroviny</b> — štěrk, cihly, prkna a spol.
      Horské suroviny musíš vytěžit tady.</div>`;
  if (load > 0) h += `<div class="rw"><div class="ric">${icon('crate',C.wood,20)}</div>
      <div class="rt"><b>Ke složení</b><i>${load} beden čeká na rampě</i></div>
      <div class="ra"><button class="mini" id="tunl">Složit</button></div></div>`;
  /* ─── vývoz dolů do města ─── */
  const ex = exportRun();
  h += `<div class="st sub">Vývoz do města</div>`;
  if (ex) {
    h += `<div class="ss">${ex.n} — ${Date.now()>=ex.end?'souprava čeká na vyzvednutí'
      :'vrátí se za '+Math.max(0,Math.ceil((ex.end-Date.now())/60000))+' min'}.</div>
      ${progBar(ex.end, ex.dur, 'exfill')}
      ${Date.now()>=ex.end?`<button class="btn go" id="exget">Vyzvednout ${fmt(ex.pay)}</button>`:''}`;
  } else {
    h += `<div class="ss">Město platí za horský kámen prémii — je to hlavní příjem hor.</div>`;
    EXPORT.tiers.forEach((t,i) => {
      const o = exportOffer(i, i*7919 + S.lvl);
      if (!o) return;
      const can = has(o.need);
      h += `<div class="rw col"><div class="rwtop">
          <div class="ric">${icon('block','#B0A498',20)}</div>
        ...
        <div class="ordbtn"><button class="mini" data-ex="${i}" ${can?'':'disabled'}>${
          can?'Naložit a vyjet':'Chybí materiál'}</button></div></div>`;
    });
  }
  h += `<div class="st sub">Zásobování z města</div>`;
  h += `<div class="rw col"><div class="rwtop">
      <div class="ric">${icon('plank',C.wood,20)}</div>
      <div class="rt"><b>Příští souprava</b>
        <i>${q} z ${capT} beden${left?` · přijede za ${Math.ceil(left/1000)} s`:''}</i></div>
      <div class="ra"><button class="mini ${S.train.auto?'':'o'}" id="tauto">${
        S.train.auto?'Automat ✓':'Automat'}</button></div></div>
    ${left?`<div class="pbar sm"><i id="trfill" style="width:${(1-left/TRAIN.dur)*100}%"></i></div>`:''}
    <div class="mkbtn">
      <button class="mini o" id="tq50" ${q+50>capT?'disabled':''}>+50</button>
      <button class="mini" id="tqmax" ${q>=capT?'disabled':''}>+${capT-q}</button>
    </div></div>`;
  return h;
}
...
function bindDetail() {
  ...
  if (q('tunl'))  q('tunl').onclick  = () => trainUnload();
  if (q('exget')) q('exget').onclick = () => { claimExport(); renderDetail(); };
  dbody.querySelectorAll('[data-ex]').forEach(b => b.onclick = () => {
    startExport(+b.dataset.ex); renderDetail(); });
  if (q('tauto')) q('tauto').onclick = () => { S.train.auto = !S.train.auto;
    toast(S.train.auto?'Vlak bude jezdit plný':'Automat vypnut'); save(); renderDetail(); };
  if (q('tq50'))  q('tq50').onclick  = () => { trainQueue(50); renderDetail(); };
  if (q('tqmax')) q('tqmax').onclick = () => { trainQueue(trainCap()); renderDetail(); };
...
function upgradeSheet(id) {
  const n = S.nodes[id], d = nodeDef(id), u = nextStUp(id), R = RL(d.res);
...
function scrStavby() {
  const rows = PIDS.filter(visible).map(id => {
    ...
    if (!isOwned(id)) sub = (p.reqLvl&&S.lvl<p.reqLvl) ? `odemkne se na LVL ${p.reqLvl}` : `nekoupeno · ${fmt(p.cost)}`;
      ...
      : isTrain(id) ? `souprava ${S.train.q} / ${trainCap()}`
...
function bindRows() {
  ...
  if ($('mord'))  $('mord').onclick  = () => { mtnOrderDeliver(); scrZakazky(); };
  if ($('mordx')) $('mordx').onclick = () => { mtnOrderRefresh(true); scrZakazky(); };
...
function scrObchod() {
  ...
  const list = ALL.filter(r => r.tier===0 || (r.tier===1&&anyFactory())
    || (r.tier===2 && (S.res[r.k]>0 || Object.keys(NODE_DEF).some(i=>S.nodes[i].lvl>=4)))
    || (r.tier===3 && showIron));
  opn(`<div class="st">Obchod</div>
    ...
    ${list.map(r=>{
      ...
      return `<div class="rw"><div class="ric">${icon(r.ic,r.c,20)}</div>
        ...
        <i>${S.res[r.k]} / ${capOf(r.k)} ks</i>${capBar(S.res[r.k],Math.max(1,capOf(r.k)))}</div>
        <div class="ra">
          ...
          <button class="mini" data-buy="${r.k}" data-a="10" ${
            S.money<buy*10||S.res[r.k]+10>capOf(r.k)?'disabled':''}>+10</button>
        </div></div>`;}).join('')}
  ...
  sbody.querySelectorAll('[data-sell]').forEach(b=>b.onclick=()=>{
    ...
    if (a<1) return; S.res[k]-=a; S.money+=a*r.price; bump('sold', a);
  ...
  sbody.querySelectorAll('[data-buy]').forEach(b=>b.onclick=()=>{
    ...
    const a=Math.min(10, capOf(k)-S.res[k], Math.floor(S.money/price));
    if (a<1) { toast('Nemáš peníze nebo místo ve skladu'); return; }
    S.res[k]+=a; S.money-=a*price; bump('bought', a);
...
function scrRozvoj() {
  ...
  PIDS.filter(visible).forEach(id => {
    ...
    if (isTrain(id)) { rows.push({ ic:swatch('#8A6E4A'), n:'Nádražní sklad',
      lv: st.done?`${trainCap()} beden`:'staví se',
      s: st.done?`souprava ${S.train.q} / ${trainCap()}${st.mload?` · ${st.mload} ke složení`:''}`
        :`fáze ${st.phase} / ${TRAIN.ph.length}`,
...
function scrZakazky() {
  ...
  opn(`<div class="st">Zakázky <span class="tag">${S.active.length} / ${orderSlots()}</span>
    ...
    ${inMtn() ? (()=>{ const o = mtnOrder(); const ex = exportRun();
      return `<div class="st sub">Kamenické zakázky</div>
        ${o?`<div class="rw col"><div class="rwtop">
            <div class="ric">${icon(RL(o.k).ic,RL(o.k).c,20)}</div>
            <div class="rt"><b>${o.q}× ${RL(o.k).l.toLowerCase()}</b>
              <i>máš ${S.res[o.k]||0} · +${fmtN(o.xp)} XP</i></div>
            <div class="ra"><b class="pay">${fmt(o.pay)}</b></div></div>
          <div class="ordbtn"><button class="mini" id="mord" ${
            (S.res[o.k]||0)<o.q?'disabled':''}>Odevzdat</button>
            <button class="mini o" id="mordx">Jiná</button></div></div>`
          :'<div class="ss">Zatím žádná — postav horský sklad a dílnu.</div>'}
        ${trainBuilt()?`<div class="st sub">Vývoz do města</div>
          <div class="rw"><div class="ric">${icon('block','#B0A498',20)}</div>
            <div class="rt"><b>Nákladní vlak ${ex?(Date.now()>=ex.end
                ?'<span class="tag ok">přijel</span>':'<span class="tag sky">na cestě</span>')
              :'<span class="tag">na nádraží</span>'}</b>
            <i>${ex?`${ex.n} · ${Date.now()>=ex.end?'čeká na vyzvednutí'
              :Math.ceil((ex.end-Date.now())/60000)+' min'}`:'čeká na náklad'}</i>
            ${ex?`<div class="pbar sm"><i class="exbar" style="width:${
              Math.max(0,Math.min(100,(1-(ex.end-Date.now())/ex.dur)*100))}%"></i></div>`:''}</div>
            <div class="ra"><button class="mini" data-p="${trainId()}">Otevřít</button></div></div>`:''}`;
    })() : ''}
...
const PLAT_INFO = {
  ...
  24:{ teaser:'Za hřebenem.', desc:'Horská osada. Chaty, které konečně vydělávají.',
      lines:[['Chata',fmt(BUILDINGS.chata.rent)+'/min'],['Čtyři parcely','a lanovka k nim']] },
