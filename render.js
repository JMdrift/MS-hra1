...
function drawBuilding(k) {
  ...
  if (PARC[k].mtrain) {
    if (st.phase < 1) { emptyPlot(p.gx,p.gy,sp); return; }
    mtnTrain(p.gx,p.gy,st.phase,st.hl,sp,st.mload); return;
  }
  if (isPumpSt(k)) {
...
/* nádražní sklad v horách */
function mtnTrain(gx,gy,ph,hl,sp,load) {
  sp = sp||3; const w = sp-.4, x = gx+.2, y = gy+.2;
  if (ph>=1) { shadow(gx+.15,gy+.15,sp-.3,sp-.3); box(x,y,w,w,0,EL+5,'#6E6A62');
    for (let i=0;i<2;i++) box(x+.1,y+w*(.16+i*.5),w-.2,.1,EL+5,EL+7,'#8E959C'); }
  if (ph>=2) { box(x+w*.06,y+w*.7,w*.6,w*.24,EL+5,EL+26,'#8A6E4A');
    gableR(x+w*.03,y+w*.67,w*.66,w*.3,EL+26,EL+36,'#5E6470'); }
  if (ph>=3) {
    box(x+w*.72,y+w*.1,.16,.16,EL+5,EL+40,C.steel);
    box(x+w*.4,y+w*.14,w*.36,.12,EL+36,EL+40,C.steel);
    const n = Math.min(3, Math.ceil((load||0)/80));
    for (let i=0;i<n;i++) box(x+.16+i*.46,y+w*.18,.4,.32,EL+7,EL+18,
      ['#C4503C','#4E6E92','#8FA45C'][i]);
  }
}
...
function drawWorld(only) {
  ...
  const objs = [];
  if (!only) {
    DECO.forEach(d => { if (!inWorldXY(d.x,d.y)) return;
    ...
    [[1, isNuke()?'#7FC24A':'#A8B0A6'], [5, '#8FB6C6'], [7, '#C0A070']].forEach(([pi,col]) => {
...
function draw() {
  ...
  if (MODE.v === 'map') { if (pulse % 2 === 0) stepAgents();
    ...
    PIDS.forEach(k => {
      ...
      if (!isOwned(k)) {
      ...
      } else if (isPort(k) || PARC[k].mtrain) {
        const nm = isPort(k) ? PORT[PARC[k].port].name : 'Nádražní sklad';
        if (!st.done) { if (big||isTarget) pill(p.gx,p.gy,sp,nm,C.amber,'#1B1305',9,5);
          else lockMark(p.gx,p.gy,sp,C.amber,0,'+'); }
        else if (isPort(k)) { const v = voyage(PARC[k].port);
        ...
        } else if (big) pill(p.gx,p.gy,sp,`${S.train.q} / ${trainCap()}`,
          'rgba(14,18,16,.88)','#C0A88E',9,5);
